...
function setupAutoUpdater(opts) {
  ...
  autoUpdater.on("update-available", (info) => {
    ...
    // Client log: idle v1.0.22 then downloading v1.0.14 — refuse downgrades explicitly.
  ...
  function resolveDownloadedInstaller() {
    const updaterCache = path.join(app.getPath("localAppData"), "@tshirtpoint-desktop-updater");
    const pending = path.join(updaterCache, "pending");
    const candidates = [];
    for (const dir of [pending, updaterCache]) {
...
if (-not $ok) {
  ...
  if (Test-Path $zipPath) { $zip = $zipPath }
    ...
    $zip = Get-ChildItem $dir -Filter 'TshirtPrinterOperator-Setup-*.zip' -File -ErrorAction SilentlyContinue |
