...
async function runSourceForCategory(
  ...
  const variantBudget = Math.min(MAX_VARIANT_ATTEMPTS_PER_RUN, tags.tags.length * 2);
  ...
  for (let attempt = 0; attempt < variantBudget && downloaded < needed; attempt++) {
    ...
    const rawLimit = Math.min(MAX_RAW_LIMIT, Math.max(stillNeeded * RAW_LIMIT_MULTIPLIER, MIN_RAW_LIMIT));
