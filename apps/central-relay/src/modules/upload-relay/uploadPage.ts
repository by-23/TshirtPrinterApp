 ...
 * фото" QR (Этап 9, `uploadMode: "relay"`) — plain HTML/JS (no React/build
 * step) since it's served directly by Fastify to an unknown phone browser.
 * Posts the chosen photo as multipart to `POST /upload/:token/photo` on the
 * same origin. Re-encodes via canvas first so Android JPEGs that browsers
 * display but libvips rejects (`Invalid SOS parameters`) arrive as a
 * clean sequential JPEG. Falls back to the original file if the phone
 * cannot re-encode (HEIC on some browsers — handled server-side).
 * Mirrors `apps/point-server/src/modules/ai/uploadPage.ts`
 * (own-wifi mode) — kept independently per-app rather than shared, same as
 * other central-relay/point-server literal duplication (see
  ...
  <script>
    ...
    submitBtn.addEventListener("click", function () {
      ...
      preparePhoto(chosenFile).then(function (photo) {
        ...
        return fetch("/upload/" + token + "/photo", { method: "POST", body: formData });
      })
        .then(function (res) {
          if (res.ok) {
